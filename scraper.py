...
{visitor_slot}
        ...
        <div class="match-connector">&rarr;</div>
      </div>\n"""
    if open_group:
        html += "    </div>\n"
    return html
...
CSS = """
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .section-title { font-size: 18px; font-weight: bold; text-align: center; color: #f1c40f; padding: 10px 0 4px; letter-spacing: 1px; }
  ...
  .bracket-title { font-size:15px; font-weight:bold; text-align:center; color:#e0e0e0; margin:18px 0 10px; padding:8px; background:#0f3460; border-radius:6px; }
  ...
  .match-num { width:28px; background:#0a1931; display:flex; align-items:center; justify-content:center; color:#888; font-size:10px; flex-shrink:0; }
  ...
  .match-connector { width:30px; display:flex; align-items:center; justify-content:center; color:#444; font-size:18px; flex-shrink:0; }
  .legend { display:flex; gap:14px; justify-content:center; flex-wrap:wrap; padding:10px; font-size:11px; color:#aaa; margin-bottom:14px; }
...
<div id="{slug}" class="section">
  ...
  <div class="deadline">Area deadline: {deadline} &nbsp;&middot;&nbsp; Sub-State R1: {r1_date} &nbsp;&middot;&nbsp; Area winner hosts runner-up</div>
...
def build_html(all_data, generated_at):
    ...
    for i, cls in enumerate(CLASSIFICATIONS):
        ...
        sections_html += build_section(cls, all_data[cls["slug"]])
...
<p class="subtitle">Live area standings &amp; projected sub-state brackets &middot; Data: scorbord.com &middot; Structure: AHSAA Sports Book</p>
...
def main():
    ...
    with open("index.html", "w", encoding="utf-8") as f:
        f.write(html)
    print(f"\nDone! index.html written ({len(html):,} bytes)")
